...
//逆変換を使わないアフィン変換（悪い例）
//g++ -o 022 022.cpp `pkg-config --cflags opencv` `pkg-config --libs opencv`
...
int main(void)
{
    ...
    for (int y = 0; y < src.rows; y++) {
        for (int x = 0; x < src.cols; x++) {
            ...
            x1 = (int)(p1(0, 0) + 0.5);     // 変換後画像の座標
            y1 = (int)(p1(1, 0) + 0.5);
 
            if (x1 >= 0 && x1 < dst.cols && y1 >= 0 && y1 < dst.rows) {
                // 座標(x1,y1)が変換後画像の範囲内であれば画素値を設定
                dst(y1, x1) = src(y, x);
            }
        }
    }
