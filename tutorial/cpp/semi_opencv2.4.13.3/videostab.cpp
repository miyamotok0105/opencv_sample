//ブレの補正
//g++ -o videostab videostab.cpp `pkg-config --cflags opencv` `pkg-config --libs opencv`
//./videostab ../../../img/shibuya.mp4
...
void run()
{
    ...
    while (!(stabilizedFrame = stabilizedFrames->nextFrame()).empty())
    {
        ...
        {
            if (!writer.isOpened())
                writer.open(outputPath, CV_FOURCC('M','P','4','V'), outputFps, stabilizedFrame.size());
